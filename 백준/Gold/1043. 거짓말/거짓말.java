...
public class Main{
    ...
    static int[] truth;
    static int[] parent;
    static List<List<Integer>> partyPeople;
    ...
    public static void main(String[] args) throws Exception{
        ...
        for(int i=0; i<m; i++){
            ...
            for(int j=0; j<len; j++){
                ...
                partyPeople.get(i).add(id); //i번째 파티에 참석하는 사람
            }
        }
        ...
        //같이 파티에 있었던 사람 union
        for(int i=0; i<m; i++){
            for(int j=0; j<partyPeople.get(i).size()-1; j++){
                union_node(partyPeople.get(i).get(j), partyPeople.get(i).get(j+1));
            }
        }
